  ...
  fun onEnable() {
    ...
    globalEventChannel().subscribeFriendMessages(
      ...
      suspend fun taskToday(
        subject: Friend, length: Int, task: String
      ) {
        val data = Data[subject] ?: return
        val now = LocalTime.now().hour
        ...
        (now until now + length).forEach {
          data.activities[it] = task
        }
        data.noDisturb = true
      ...
      matching(Regex("等会我?[要会](.+?)([ ,，]*别打扰我)?")) {
        val (task) = it.destructured
        taskToday(subject, 1, task)
      }
      matching(Regex("""之后(\d{1,2})小时我?[要会](.+?)([ ,，]*别打扰我)?""")) {
        val (length, task) = it.destructured
        taskToday(subject, length.toInt(), task)
      }
      suspend fun taskFuture(
        ...
        if (noDisturb) {
          data.calendar[date.toString()] = UserDay(
            true,
            setOf(),
            mapOf(Pair(data.endOfDay - 1, task)),
            null
          )
          subject.sendMessage("明白了")
        } else {
      ...
      Data.forEach { (subject, data) -> launch { when (now) {
        ...
        in data.startOfDay until data.endOfDay -> {
          ...
          if (
            ...
            subject.sendMessage("${now}点到了！")
            subject.sendMessage("${data.activities[now - 1]}，完成了吗？")
            select<Unit> {
              nextMessageAsync(subject).onAwait {
                if (it.message.contentToString().contains("没")) {
                  subject.sendMessage("没法在预定的时间内完成啊")
                  subject.sendMessage("要延长一小时吗")
                  if (select<Boolean> {
                    nextMessageAsync(subject).onAwait {
                      !it.message.contentToString().contains(Regex("不|算了"))
                    }
                    onTimeout(300_000) {
                      subject.sendMessage("那我就默认为不用了")
                      false
                    }
                  }) {
                    data.activities[now] = data.activities[now - 1]!!
                    subject.sendMessageIn("好", "行")
                    subject.sendMessage("接下来加油哦")
                  }
                } else {
                  subject.sendMessageIn(
                    "那么，休息一下再继续做别的事吧",
            ...
            select<Unit> {
              nextMessageAsync(subject).onAwait {
                ...
                if (doneTasks.isNotEmpty()) {
                  ...
                  msg.contains(Regex("[玩打](游戏|.游|MC|mc|osu)|出去玩")) -> {
                    subject.sendMessage("玩得开心~")
                  }
                  msg.contains(Regex("吃|恰饭")) -> {
                    subject.sendMessage("好吃吗")
                  }
                  msg.contains(Regex("坐车|开车|在路上")) -> {
                    subject.sendMessage("赶路很辛苦吧")
                    subject.sendMessage("祝你不虚此行")
                  }
                  msg.contains(Regex("午休|休息|睡觉")) -> {
        ...
        data.endOfDay -> {
          ...
          if (
            data.noDisturb &&
            data.activities.size == 1 &&
            data.activities.containsKey(now - 1)
          ) {
            subject.sendMessage("${data.activities[now - 1]}一整天，感觉怎么样？")
          } else {
            subject.sendMessageIn(
              "回顾一下这一天吧……",
              "看看这一天你做了些什么……"
